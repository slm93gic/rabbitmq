...
public class SendMegController {
    ...
    public void sendMsgTTL(@PathVariable String message, @PathVariable String ttl) throws Exception {
        log.info("当前时间{}，发送一条时间长{}秒的TTL队列消息：{}", new Date().toString(), ttl, message);
        rabbitTemplate.convertAndSend("X", "XC", "消息来自TTL " + ttl + "毫秒的队列：" + message, msg -> {
            //发送消息的延迟时间
